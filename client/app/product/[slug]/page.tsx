...
const ProductPage = ({ params }: { params: { slug: string } }) => {
  ...
  const { data, isLoading, isError } = useGetSinglProduct({
    slug: params.slug,
  });
    ...
    <div className="bg-custom-image">
      <div className="max-w-screen-xl mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row">
          ...
          <div className="flex-1">
            <h1 className="text-2xl font-bold font-serif text-[#595478]">
              {productdat?.name}
            </h1>
            <div className="text-xl text-[#464260] font-semibold">
              Price: {productdat?.price * productdat?.quantity}Rs
            </div>
            <div className="text-green-600">
              Save: {discount} even more with extra 20%
            </div>
            <div className="text-red-500">
              Get 20% OFF use Coupon code {couponCode}
            </div>
            ...
            <div className="flex flex-row items-center gap-3 py-3">
              ...
              <div className="flex flex-row items-center gap-2  border border-gray-300 p-1 rounded-[5px]">
                <i onClick={HandleDecrease} className="cursor-pointer">
                  <MinusSVG fill={"#000"} height={20} width={20} />
                </i>
                <p className="p-1">{productdat?.quantity}</p>
                <i onClick={HandleIncreament} className="cursor-pointer">
                  <PlusSVG fill={"#000"} height={20} width={20} />
                </i>
              </div>
            </div>
            <div className="mt-4">
              <button
                ...
                className="w-full py-3 bg-button text-white font-bold rounded"
              >
                ADD TO BASKET
              </button>
            </div>
      ...
      <div className="p-4 mt-10 bg-[#f7e4d9] ">
        <div className="flex gap-8 justify-center max-w-screen-lg mx-auto">
          {Tabs?.map((tabName: string, index: number) => {
            return (
              <div
                key={index}
                className={`gap-4 cursor-pointer p-1 rounded-[3px] text-[20px] font-[500] ${
                  tabName === activeTab
                    ? "border-b-2 text-[#747373]  border-[#747373] font-[600]"
                    : "text-[#1e1d1d]"
                }`}
        ...
        <TabsContent
          productName={productdat?.name}
          tab={activeTab}
          description={productdat?.discription}
          productId={productdat?.id}
        />
      </div>
    </div>
  );
...
const Tabs = ["Description", "Reviews", "Return Policy", "Delivery Policy"];
