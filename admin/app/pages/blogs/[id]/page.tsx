...
import { useCreatBlog, useUpdateBlog } from "@/app/network-request/mutations";
...
const BlogUpdate = ({ params }: { params: { id: string } }) => {
  const { data: singleBlog, isError: iserror, isLoading } = useGetSinglBlog({
    id: params.id,
  });
  const { data, isError } = useGetCategories();
  const Blogupdate = useUpdateBlog();
  ...
  return (
    <>
      ...
      <div className="w-full md:px-4 md:ml-4 md:pl-0">
        <form className="w-full h-[calc(100vh-6rem)] overflow-hidden   rounded-md">
